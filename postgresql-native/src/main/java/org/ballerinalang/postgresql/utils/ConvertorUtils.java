        ...
        public static PGobject convertCustomType(Object value) throws SQLException, ApplicationError {
            ...
            if (type.getTag() == TypeTags.OBJECT_TYPE_TAG) {
                if (value instanceof BObject) {
                    BObject typedValue = (BObject) value;
                    Object innerObject = typedValue.get(org.ballerinalang.sql.Constants.TypedValueFields.VALUE);
                    Map<String, Object> customRecord = ConversionHelperUtils.getRecordType(innerObject);
                    if (customRecord.containsKey(Constants.Custom.TYPE) && 
                            customRecord.containsKey(Constants.Custom.VALUES)) {
                        String typeName = customRecord.get(Constants.Custom.TYPE).toString();
                        ArrayList<Object> objectArray = ConversionHelperUtils.getArrayType((BArray) customRecord
                        .get(Constants.Custom.VALUES));
                    ...
                    throw new ApplicationError("Unsupported Object Type for PostgreSQL Custom Datatype");
                }
            } else {
