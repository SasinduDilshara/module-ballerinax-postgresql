...
public type ArrayFunctionRecord2 record {
  int row_id;
  int?[]? smallint_array;
  int?[]? int_array;
  int?[]? bigint_array;
...
@test:Config {
    groups: ["functions"],
    dependsOn: [testArrayFunctionInParameter]
}
function testArrayFunctionInParameter2() returns error? {
    int rowId = 3;
    float float1 = 122.43;
    ...
    if (qResult is ()) {
        ...
        record {|record {} value;|}? data = check qResult.next();
        record {}? result1 = data?.value;
        ArrayFunctionRecord2 expectedDataRow = {
            row_id: 1,
            smallint_array: [12, 232],
            int_array: [1, 2, 3],
            bigint_array: [100000000, 200000000, 300000000],
            numeric_array: <decimal[]>[12.323, 232.21],
    ...
    if (qResult is ()) {
        ...
        record {|record {} value;|}? data = check qResult.next();
        record {}? result2 = data?.value;
        ArrayFunctionRecord2 expectedDataRow2 = {
            row_id: 2,
            smallint_array: (),
            int_array: (),
            bigint_array: (),
    ...
    if (qResult is ()) {
        ...
        record {|record {} value;|}? data = check qResult.next();
        record {}? result3 = data?.value;
        ArrayFunctionRecord2 expectedDataRow3 = {
            row_id: rowId,
...
function testRangeFunctionOutParameter() returns error? {
    ...
    test:assertTrue(tsrangeOutValue.get(TimestampRangeToCivil) is TimestampRangeToCivil, "Tsrange Datatype Doesn't Match");
    test:assertTrue(tstzrangeOutValue.get(TimestamptzRangeToCivil) is TimestamptzRangeToCivil, "Tstzrange Datatype Doesn't Match");
    test:assertTrue(daterangeOutValue.get(DateRangeToCivil) is DateRangeToCivil, "Daterange Datatype Doesn't Match");
}
