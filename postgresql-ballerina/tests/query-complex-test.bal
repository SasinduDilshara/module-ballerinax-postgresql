...
public type RangeRecord3 record {
  int row_id;
  TimestampRangeToCivil? tsrange_type;
  TimestamptzRangeToCivil? tstzrange_type;
  DateRangeToCivil? daterange_type;
};
...
isolated function validateRangeTableResult5(record{}? returnData) {
    if (returnData is ()) {
        ...
        test:assertEquals(returnData["row_id"], 1);
        test:assertTrue(returnData["tsrange_type"] is TimestampRangeToCivil);
        test:assertTrue(returnData["tstzrange_type"] is TimestamptzRangeToCivil);
        test:assertTrue(returnData["daterange_type"] is DateRangeToCivil);
    } 
}
